...
import { getStoredPairingCode, usePairing } from "./pairing";
...
export default function DAWStudio({ onExit, plugin, audioProject }: DAWStudioProps) {
  const { isSignedIn, userId, pair, unpair, generate } = usePairing();
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        syncRef.current = new ProjectSyncManager(database, new HttpSyncTransport(getStoredPairingCode), setSyncState, (incoming) => {
    ...
    <div className="h-screen bg-neutral-950 text-neutral-200 flex flex-col font-sans selection:bg-orange-500/30">
      ...
      <div className="flex-1 min-h-0 flex">
        ...
        <aside className="w-72 shrink-0 border-l border-neutral-800 flex flex-col">
          <SyncPanel state={syncState} signedIn={Boolean(isSignedIn)} pairingCode={userId}
            onPair={pair} onUnpair={unpair} onGenerate={generate}
