# Consuming ORANGEJUCE UI in web apps

Read `artifacts/orangejuce-ui/docs/AGENTS.md` first. This guide covers
...
`artifacts/orangejuce-ui/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/orangejuce-ui/styles.css";
```
...
`node_modules/@workspace/orangejuce-ui/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/orangejuce-ui/components/ui/button";
import { cn } from "@workspace/orangejuce-ui/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/orangejuce-ui/hooks/use-toast";
```
...
`@workspace/orangejuce-ui/components/ui/button`. Run the app's typecheck
