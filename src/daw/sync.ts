...
export class HttpSyncTransport implements SyncTransport {
  ...
  /** `getCode` is read fresh on every request rather than captured once --
   *  the pairing code can change (pair/unpair) after this transport is
   *  constructed, and there is no cookie/session to carry it implicitly
   *  the way Clerk's sign-in did. */
  constructor(private readonly getCode: () => string | null, private readonly base = "/api/daw-sync") {}
  private async request(path: string, init?: RequestInit) {
    const code = this.getCode();
    if (!code) throw new Error("Pair this device before enabling sync.");
    const response = await fetch(`${this.base}${path}`, {
      ...init,
      headers: { ...init?.headers, "X-Sync-Code": code },
    });
    if (response.status === 401) throw new Error("Pair this device before enabling sync.");
