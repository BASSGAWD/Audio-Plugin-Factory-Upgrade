# Migrating Expo UI to ORANGEJUCE UI

Read `artifacts/orangejuce-ui/docs/AGENTS.md` and
`artifacts/orangejuce-ui/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/orangejuce-ui/lib/native-theme` and
  `@workspace/orangejuce-ui/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/orangejuce-ui/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/orangejuce-ui` and retains only
