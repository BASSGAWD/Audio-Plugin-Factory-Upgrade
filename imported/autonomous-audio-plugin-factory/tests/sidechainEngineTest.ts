...
import { scaffoldNativeProject, validateNativePlugin } from "../server/nativeBuild";
...
int main() {
  ...
  try {
    execFileSync("g++", ["-std=c++20", "-O2", harnessPath, "-o", harnessBinary], { stdio: "pipe" });
    execFileSync(harnessBinary, [], { stdio: "pipe" });
    nativeBehavior = true;
  } catch {}
  ...
  try {
    execFileSync("g++", ["-std=c++20", literalHarnessPath, "-o", literalHarnessBinary], { stdio: "pipe" });
    execFileSync(literalHarnessBinary, [], { stdio: "pipe" });
